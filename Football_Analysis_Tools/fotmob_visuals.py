...
def plot_match_shotmap(ax, match_id:int, homecolor, awaycolor):
    ...
    # Check for own goals by the home team and add them to the away team's goals
    Home_own_goals = h_data[(h_data['eventType'] == 'Goal') & (h_data['isOwnGoal'] == True)]
    Away_goals = a_data[(a_data['eventType'] == 'Goal') & (a_data['isOwnGoal'] == False)]
    Away_goals = pd.concat([Away_goals, Home_own_goals])

    # Check for own goals by the away team and add them to the home team's goals
    Away_own_goals = a_data[(a_data['eventType'] == 'Goal') & (a_data['isOwnGoal'] == True)]
    Home_goals = h_data[(h_data['eventType'] == 'Goal') & (h_data['isOwnGoal'] == False)]
    Home_goals = pd.concat([Home_goals, Away_own_goals])
    ...
    ax.scatter(105 - Home_shots.x,
               70 - Home_shots.y,
               c=homecolor,
               s=Home_shots.expectedGoals * 120 ,
               marker = 'o',
               alpha = .8,
               label = 'Shots'
               )
    ...
    ax.scatter(105 - Home_goals.x,
               70 - Home_goals.y,
               c="#f28482",
               s=Home_goals.expectedGoals * 120 ,
               marker='o',
               alpha = .8,
               label = 'Goal'
               )
    ...
    ax.scatter( Away_shots.x,
                Away_shots.y,
                c=awaycolor,
                s=Away_shots.expectedGoals * 120 ,
                alpha = .8,
                label = 'Shot',
                marker = 'o'
                )
    ...
    ax.scatter( Away_goals.x,
                Away_goals.y,
                c="#f28482",
                s=Away_goals.expectedGoals * 120 ,
                marker='o',
                alpha= .8,
