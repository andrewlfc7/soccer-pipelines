...
def plot_event_clusters_away(ax,k, data, teamid, filter=None):
    ...
    cluster_colors = ['#b18fcf', '#ef476f', '#26547c', '#e63946', '#ffb288', '#e5989b']
    # get counts of each cluster in the dataframe
    cluster_counts = df['cluster'].value_counts()

    # get the most common and least common clusters
    most_common_cluster = cluster_counts.idxmax()
    least_common_cluster = cluster_counts.idxmin()

    # loop through each row in the dataframe and plot the pass with corresponding alpha value
    for i in range(k):
        cluster_df = df[df['cluster'] == i]
        if i == most_common_cluster:
            alpha = 1
        elif i == least_common_cluster:
            alpha = 0.2
        else:
            alpha = 0.6
        color = cluster_colors[i]
        for x in range(len(cluster_df)):
            pitch.arrows(xstart=cluster_df.iloc[x]['x'], ystart=cluster_df.iloc[x]['y'], xend=cluster_df.iloc[x]['endX'], yend=cluster_df.iloc[x]['endY'], color=color,width=2.5 ,alpha=alpha, ax=ax)
...
def plot_clusters_event_opp_half(ax, k, data, teamid, filter=None):
    data = data[data['is_open_play'] == True]
    df = data[data['teamId'] == teamid].copy()
    if filter:
        df = df.query(filter)

    df = df[df['x'] > 50].reset_index(drop=True)

    df = df[df['endX'] > 50].reset_index(drop=True)
    ...
    for i in range(k):
        ...
        color = cluster_colors[i]
        for x in range(len(cluster_df)):
            pitch.arrows(xstart=cluster_df.iloc[x]['x'], ystart=cluster_df.iloc[x]['y'], xend=cluster_df.iloc[x]['endX'], yend=cluster_df.iloc[x]['endY'], color=color, width=2, alpha=alpha, ax=ax)
