...
match_date = get_match_date_and_id(8650)[0]

script_text = scraper.get_match_event_data(date=match_date)


data = scraper.data_preprocessing(script_text)
...
def custom_events(data):
    ...
    # We define a set that covers all (I think) recovery ball actions:
    chances_set = set(
        ['keyPassLong', 'keyPassShort', 'keyPassCross', 'keyPassCorner', 'keyPassThroughball', 'keyPassFreekick',
         'bigChanceCreated', 'passkey'])
    data = data.copy()
    data['key_pass'] = False
    for index, row in enumerate(data['satisfiedEventsTypes']):
        set_element = set(row)
    ...
    # We define a set that covers all (I think) recovery ball actions:
    assist_set = set(
        ['assistCross', 'assistCorner', 'assistThroughball', 'assistFreekick', 'assistThrowin', 'assistOther', 'assist'])
    data = data.copy()
    data['assist'] = False
    for index, row in enumerate(data['satisfiedEventsTypes']):
        set_element = set(row)
    ...
    # We define a set that covers all (I think) recovery ball actions:
    FinalThirdPasses_set = set(['successfulFinalThirdPasses'])
    data = data.copy()
...
match_data = custom_events(data)
import datetime
# date = datetime.datetime.now().strftime("%Y-%m-%d")

match_data.to_csv(f'../Data/process_data/match_data{match_date}.csv')
