...
if current_competition == 'Premier League':
    query = """SELECT * FROM opta_event_data WHERE ("teamId" = 26 OR "match_id" IN (SELECT "match_id" FROM opta_event_data WHERE "teamId" = 8650)) AND competition = 'Premier League'"""
    comp_name = 'Premier League'
else:
    query = """SELECT * FROM opta_event_data WHERE ("teamId" = 26 OR "match_id" IN (SELECT "match_id" FROM opta_event_data WHERE "teamId" = 8650))"""
    comp_name = 'All Competition'
...
data[data['teamId'] == 8650]['matchId'].value_counts()
...
def calculate_match_shots_stats(data, teamId):
    ...
    # --- Liverpool
    xG = liv_data.groupby(['matchId'])['expectedGoalsOnTarget'].sum().reset_index().rename(columns={'expectedGoalsOnTarget': 'xGOT_liv'})
    npxG = liv_data[liv_data['situation'] != 'Penalty'].groupby(['matchId'])['expectedGoals'].sum().reset_index().rename(columns={'expectedGoals': 'npxG_liv'})
    openplay_xG = liv_data[liv_data['situation'] == 'RegularPlay'].groupby(['matchId'])['expectedGoals'].sum().reset_index()
    openplay_shots = liv_data[liv_data['situation'] == 'RegularPlay'].groupby(['matchId']).size().reset_index(name='shotCount')
    openplay_xG = openplay_xG.merge(openplay_shots, on=['matchId'])
    openplay_xG['xG_per_shot_Liv'] = openplay_xG['expectedGoals'] / openplay_xG['shotCount']
    openplay_xG = openplay_xG.rename(columns={'expectedGoals': 'openplay_xG_liv'})
    setpiece_xG = liv_data[liv_data['situation'] == 'SetPiece'].groupby(['matchId'])['expectedGoals'].sum().reset_index().rename(columns={'expectedGoals': 'setpiece_xG_liv'})

    # --- Opponents
    npxG_Opp = opponents_data[opponents_data['situation'] != 'Penalty'].groupby(['matchId', 'teamId'])['expectedGoals'].sum().reset_index().rename(columns={'expectedGoals': 'npxG_Opp'})
    openplay_xG_Opp = opponents_data[opponents_data['situation'] == 'RegularPlay'].groupby(['matchId', 'teamId'])['expectedGoals'].sum().reset_index()
    openplay_shots_Opp = opponents_data[opponents_data['situation'] == 'RegularPlay'].groupby(['matchId', 'teamId']).size().reset_index(name='shotCount')
    openplay_xG_Opp = openplay_xG_Opp.merge(openplay_shots_Opp, on=['matchId', 'teamId'])
    openplay_xG_Opp['OP_xG/shots_Opp'] = openplay_xG_Opp['expectedGoals'] / openplay_xG_Opp['shotCount']
    openplay_xG_Opp = openplay_xG_Opp.rename(columns={'expectedGoals': 'openplay_xG_Opp'})

    setpiece_xG_Opp = opponents_data[opponents_data['situation'] == 'SetPiece'].groupby(['matchId', 'teamId'])['expectedGoals'].sum().reset_index().rename(columns={'expectedGoals': 'setpiece_xG_Opp'})

    # Merge dataframes for Liverpool
    liv_merged = npxG.merge(setpiece_xG, on=['matchId']).merge(openplay_xG, on=['matchId']).merge(xG, on=['matchId'])
...
#%%
calculate_match_shots_stats(data,8650)[1]
...
logo_ax = fig.add_axes([0.08, .82, 0.08, 0.08], frameon=False)
...
sns.scatterplot(data=stats_opp, x='npxG_Opp', y=index, c='#43B8AA', edgecolor='#43B8AA', s=20, marker='o', alpha=.2,
                ax=axs[1])
sns.scatterplot(data=stats_opp[stats_opp['matchId'] == '4193509'], x='npxG_Opp', y=index, c='#660708', edgecolor='k', s=20,
...
sns.scatterplot(data=stats_opp, x='openplay_xG_Opp', y=index, c='#43B8AA', edgecolor='#43B8AA', s=20, marker='o', alpha=.2,
                ax=axs[5])
sns.scatterplot(data=stats_opp[stats_opp['matchId'] == '4193509'], x='openplay_xG_Opp', y=index, c='#660708', edgecolor='k',
...
sns.scatterplot(data=stats_opp, x='setpiece_xG_Opp', y=index, c='#43B8AA', edgecolor='#43B8AA', s=20, marker='o', alpha=.2,
                ax=axs[3])
sns.scatterplot(data=stats_opp[stats_opp['matchId'] == '4193509'], x='setpiece_xG_Opp', y=index, c='#660708', edgecolor='k',
...
sns.scatterplot(data=stats_opp, x='OP_xG/shots_Opp', y=index, c='#43B8AA', edgecolor='#43B8AA', s=20, marker='o', alpha=.2,
                ax=axs[7])
sns.scatterplot(data=stats_opp[stats_opp['matchId'] == '4193509'], x='OP_xG/shots_Opp', y=index, c='#660708', edgecolor='k',
...
sns.scatterplot(data=match_stats_opp[(match_stats_opp['teamId']!=26) & (match_stats_opp['matchId'] == 1729284)], x='xThreat_gen', y=index, c='#660708', edgecolor='k', s=20, marker='o', alpha=.88, ax=axs[9])
sns.scatterplot(data=match_stats[(match_stats['teamId']==26) & (match_stats['matchId'] == 1729284)], x='xThreat_gen', y=index, c='#660708', edgecolor='k', s=20, marker='o', alpha=.88, ax=axs[8])


sns.scatterplot(data=ppda[ppda['teamId']==26], x='PPDA', y=index, c='#43B8AA', edgecolor='#43B8AA', s=20, marker='o', alpha=.2,
                ax=axs[10])

sns.scatterplot(data=ppda[(ppda['teamId']==26) & (ppda['matchId'] == 1729284)], x='PPDA', y=index, c='#660708', edgecolor='k', s=20, marker='o', alpha=.88, ax=axs[10])



sns.scatterplot(data=match_stats[(match_stats['teamId']==26) & (match_stats['matchId'] == 1729284)], x='defensive_line_height', y=index, c='#660708', edgecolor='k', s=20, marker='o',zorder=4, alpha=.88,
                ax=axs[11])
...
sns.scatterplot(data=possession_metric[(possession_metric['teamId']==26) & (possession_metric['matchId'] == 1729284.0)], x='possession_metric', y=index, c='#660708', edgecolor='k', s=20, marker='o',zorder=4, alpha=.88,
                ax=axs[13])
...
sns.scatterplot(data=match_stats[(match_stats['teamId']==26) & (match_stats['matchId'] == 1729284)], x='pass_success_rate', y=index, c='#660708', edgecolor='k', s=20, marker='o', alpha=.88, ax=axs[14])
...
sns.scatterplot(data=match_stats_opp[match_stats_opp['teamId']!=26], x='pass_success_rate', y=index, c='#43B8AA', edgecolor='#43B8AA', s=20, marker='o', alpha=.2,
                ax=axs[15])
sns.scatterplot(data=match_stats_opp[(match_stats_opp['teamId']!=26) & (match_stats_opp['matchId'] == 1729284)], x='pass_success_rate', y=index, c='#660708', edgecolor='k', s=20, marker='o', alpha=.88, ax=axs[15])
